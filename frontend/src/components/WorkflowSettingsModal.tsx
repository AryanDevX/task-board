          ...
          <ul className={styles.transitionList}>
            {transitions.map(t => (
              <li key={t.id} className={styles.transitionItem}>
                <span className={styles.transitionLabel}>{getColumnName(t.fromColumnId)} <span className={styles.transitionArrow}>→</span> {getColumnName(t.toColumnId)}</span>
                <button onClick={() => handleDelete(t.id)} className={styles.transitionRemoveButton}>Remove</button>
              </li>
            ))}
            {transitions.length === 0 && <li className={sharedStyles.helperText}>No transitions defined yet.</li>}
          </ul>
