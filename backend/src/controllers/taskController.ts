...
export const createTask = async(req: Request, res: Response, next: NextFunction): Promise<void> => {
    try{
        if(!req.user || !req.user.userId) return next(new AppError('Unauthorized', 401));
...
export const updateTask = async(req: Request, res: Response, next: NextFunction): Promise<void> => {
    try{
        ...
        const updatedTask = await taskService.updateTask(parseInt(taskId), req.body, req.user.userId);
...
export const moveTask = async(req: Request, res: Response, next: NextFunction): Promise<void> => {
    try{
        ...
        const updatedTask = await taskService.moveTask(parseInt(taskId), req.body, req.user.userId);
...
export const deleteTask = async(req: Request, res: Response, next: NextFunction): Promise<void> => {
    try{
        ...
        const deletedTask = await taskService.deleteTask(parseInt(taskId), req.user.userId);
